...
class Escenario1TrainEnv(gym.Env):
    """Custom gym environment to train a PPO agent for Escenario 1."""
    def __init__(self):
        super().__init__()
        ...
        # Teammate: greedy_full_task (ingredient: onion, no noise)
        self.partner = GreedyFullTaskPolicy(ingredient="onion")
...
def train():
    ...
    env = make_vec_env(lambda: Escenario1TrainEnv(), n_envs=16, vec_env_cls=SubprocVecEnv)
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    print("Training PPO model for Escenario 1 for 1,000,000 steps...")
    model.learn(total_timesteps=1_000_000)
    model.save("models/ppo_escenario1")
    print("Saved model to models/ppo_escenario1.zip!")
