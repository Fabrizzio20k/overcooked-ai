...
class Escenario3TrainEnv(gym.Env):
    """Custom gym environment to train a PPO agent for Escenario 3."""
    def __init__(self):
        super().__init__()
        ...
        # Teammate: greedy_full_task with 35% random actions (sticky/noisy actions) and onion ingredient
...
def train():
    env = make_vec_env(lambda: Escenario3TrainEnv(), n_envs=4)
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    print("Training PPO model for Escenario 3 for 1,000,000 steps...")
    model.learn(total_timesteps=1_000_000)
    model.save("ppo_escenario3")
    print("Saved model to ppo_escenario3.zip!")
